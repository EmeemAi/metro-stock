...
// Función auxiliar para inicializar iconos de forma segura (sin que crashee el script)
function safeCreateIcons(parentNode = document) {
    try {
        ...
        console.warn("Lucide Icons falló al cargar; continuando sin refrescar iconos.", err);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Inicializar Iconos Lucide con Blindaje
    safeCreateIcons();
...
function renderTable() {
    ...
    if (filtered.length === 0 && !appState.loading) {
        ...
        // Reinicializar iconos SOLO en el tbody para no escanear toda la página (Blindado)
...
function openModalEdit(id) {
    ...
    // Cargar iconos una sola vez al final (Blindado)
...
function openModalFicha(id) {
    ...
    // Inicializar iconos una vez terminada la tabla (Blindado)
    safeCreateIcons(modal);
...
function renderCharts() {
    ...
    appState.data.forEach(item => {
        if(item.estado && statesData[item.estado] !== undefined) {
    ...
    // Chart 1: Estados (Blindado contra ausencia de Chart.js)
    ...
    } catch(err) { console.error("Error al renderizar gráfico de estados:", err); }
    ...
    } catch(err) { console.error("Error al renderizar gráfico de instrumentos:", err); }
